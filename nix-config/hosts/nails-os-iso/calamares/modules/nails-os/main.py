...
def make_hardware_config(
    ...
    EFI:  /boot → FAT32 vfat  (EFI system partition)
    BIOS: /boot → ext4        (unencrypted, GRUB-readable)
...
def run():
    ...
    # Fallback: scan lsblk for the /boot device mounted at <root>/boot.
    if not boot_device:
    ...
    if not boot_device:
        mode_label = "EFI" if efi_mode else "BIOS /boot"
        return (
            _("Installation error"),
            _("Could not detect {} partition. Check partition layout.").format(
                mode_label
            ),
        )
    if not luks_device:
    ...
    if not boot_uuid:
        mode_label = "EFI" if efi_mode else "/boot"
        return (
            _("Installation error"),
            _("Could not read UUID of {} partition.").format(mode_label),
        )
    if not luks_uuid:
        return (_("Installation error"), _("Could not read UUID of LUKS partition."))

    # For BIOS installs, derive the parent disk device for grub.device.
    if not efi_mode:
        grub_disk_device = get_disk_device(boot_device)
